...
if (isset($_POST['update_client_account'])) {
    ...
    if (!empty($profile_pic)) {
        ...
        $query = "UPDATE iB_clients SET name=?, national_id=?, phone=?, email=?, address=? WHERE client_number = ?";
        $stmt = $mysqli->prepare($query);
        $rc = $stmt->bind_param('ssssss', $name, $national_id, $phone, $email, $address, $client_number);
    }
    $stmt->execute();
